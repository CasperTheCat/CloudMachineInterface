...
for i in range(backstep):
    itu = numpy.expand_dims(inVal[i], 0) 
...
dataP = inVal[Utils.offset + seqLength:].transpose()[4]
#dataT = inFeed.transpose()[5]
dataT = numpy.array(predicts)
...
Utils.MakeCSV(pairwiseErrors, "ML_{}.csv".format(Utils.TimeNow()))
Utils.MakeCSV(pairwiseErrorsAcc, "ML_{}_SIGNED.csv".format(Utils.TimeNow()))
